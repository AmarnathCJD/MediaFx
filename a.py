...
from qt6 import MyDialog, MyMessageBox
...
class VideoPlayer(QWidget):
    ...
    def __init__(self, aPath, parent=None):
        ...
        # add border to videoWidget, orange border

        def p(msg):
            print(msg)
        self.mediaPlayer.seekableChanged.connect(p)
        ...
        self.playButton.setStyleSheet("background-color: black; border: 0px solid black; margin: 0px; padding: 12px; width: 32px; height: 32px")
        ...
        jlabel_to_hold_gif = QLabel(self)
        # jlabel_to_hold_gif.setGeometry(400, 40, 4, 4)
        jlabel_to_hold_gif.setStyleSheet("background-color: black")
        mov = QMovie("o.gif")
        jlabel_to_hold_gif.setMovie(mov)
        mov.start()
        ...
        layout.addWidget(jlabel_to_hold_gif, alignment=Qt.AlignmentFlag.AlignCenter, stretch=2)
    ...
    def playFromURL(self):
        ...
        if d.exec() == QDialog.DialogCode.Accepted:
            self.myurl = d.lineEdit.text()
            self.mediaPlayer.setSource(QUrl(self.myurl))
            self.mediaPlayer.play()
            # display spin.gif on top of videoWidget
            
            from PyQt6.QtWidgets import QLabel
            from PyQt6.QtGui import QMovie

            self.movie = QMovie("spin.gif")
            self.movie.setScaledSize(self.videoWidget.size())
    ...
    def changeTitle(self, state):
        if state == QMediaPlayer.PlaybackState.PlayingState:
            self.setWindowTitle("QT6 Player")
    ...
    def contextMenuRequested(self, point):
        ...
        actionURL = menu.addAction(QIcon.fromTheme(
            "browser"), "URL from Clipboard (u)")
        actionclipboard = menu.addSeparator()
        actionYTurl = menu.addAction(
            QIcon.fromTheme("youtube"), "URL from YouTube (y)")
        actionclipboard = menu.addSeparator()
        ...
        if self.mediaPlayer.mediaStatus() == 6:
            if self.mediaPlayer.isMetaDataAvailable():
                res = str(self.mediaPlayer.metaData("Resolution")).partition(
                    "PyQt6.QtCore.QSize(")[2].replace(", ", "x").replace(")", "")
                print("%s%s" % ("Video Resolution = ", res))
                if res:
                    v = round(
                        int(res.partition("x")[0]) / int(res.partition("x")[2]))
                    if v < 1.5:
                        self.screen43()
                    else:
                        self.screen169()
...
if __name__ == '__main__':
    ...
    player = VideoPlayer('')
    player.setStyleSheet("QVideoWidget { border: 1px solid orange }")
    player.setWindowTitle("QT6 Player")
    player.setWindowIcon(QIcon.fromTheme("multimedia-video-player"))
    player.setWindowFlags(Qt.WindowType.WindowStaysOnTopHint)
